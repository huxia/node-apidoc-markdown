...
- [<%= data[group][sub][0].title %>](#<%=: (data[group][sub][0].type + data[group][sub][0].url) | replace:/[\?\/]/g,'-' | mlink %>) `<%-: data[group][sub][0].type | upcase %> <%= data[group][sub][0].url %>`
	<% }); -%>
...
<% Object.keys(data[group]).forEach(function (sub) { -%>
#### <%=: (data[group][sub][0].type + data[group][sub][0].url) | replace:/[\?\/]/g,'-' | mlink %>
